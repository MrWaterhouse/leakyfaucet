#LeakyFaucetAI v1.0.1
#Written: Mr.Waterhouse
#May 17, 2023
#
#This DNS tunnelling script. It is designed to quickly test if DNS tunnelling is possible
...
#
#Unlike the leakyfaucet scripts, the ai script is interactive. The usage is slightly different:
#Usage: python3 leakyfaucetai.py [Optional_Command]
#ie. python3 leakyfaucetai.py ai
#
...
#argument at the cli.
#
#If you do not include an option command, such as 'ai', it will prompt your for your phonenumber and domain only.
#If you specify: ai  It will also ask you for the message you wish to send to the ai server-side script. Provided you have it setup, it will send the query via API
#the server and sms the response back to you.
#
...
phone_arg = input("Phone number? ")
phone_number = str(ranNum) + str(phone_arg) + str(command_arg)

#cli_domain = sys.argv[2]
cli_domain = input("Which domain are you using? ")
listener_domain = str(cli_domain)

#Check if the ai question instance has been initiated. If yes, get the question. If not, assume dummy dns data.
if command_arg == 'ai' :
    question = input("What is your question? ")
    ai_end = str(ranNum) + str(phone_arg) + 'aind' #Create end of questions signal text
    dns_data =[phone_number]
    if len(question) > 35:  #Check if the length is too long for a single DNS query
        # Split the question into 25-character pieces
        for i, piece in enumerate(range(0, len(question), 25), start=1):
            piece = f"{ranNum}.{i}. {question[piece:piece+25]}" #Cut the piece into 25 character chunks and add session and enumerator at front
            dns_data.append(piece)
    else: #If the questions is shorter than 35 characters, add it to the dns array
        padding = "1."
        piece = f"{ranNum}.{padding} {question}"
        dns_data.append(piece)
    dns_data.append(ai_end)  #Tack phrase on the end so the parser knows not to expect more question text.
else:  #If not an ai question, insert dummy data
...
#(Technically it just needs to be equally divisible by 12)
